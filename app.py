...
st.set_page_config(page_title="Video to Slide Converter", page_icon="🎬")

st.title("🎬 Video to Slide Converter")
st.write("Upload a video and extract slides automatically using our cloud servers.")

# 1. Corrected function name: st.file_uploader
...
if uploaded_file is not None:
    ...
    if st.button("Convert to Slides"):
        with st.spinner("Processing... this might take a moment depending on video size."):
            try:
                ...
                # 3. Fixed the escape sequence warning by using a raw string for the filter
                ffmpeg_cmd = [
